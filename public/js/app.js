$(document).ready(function(){
    ...
    var contact_table = $('#contact_table').DataTable({
        ...
        ajax: {
         ...
         columns: [
             ...
             //{data: 'category', name: 'contact_category_name'},
             /* {data: 'past_debit', name: 'past_debit'}, */ 
             /* {data: 'debit_in_usd', name: 'debit_in_usd'}, */
             {data: 'debit_in_iqd_amount', name: 'debit_in_iqd_amount'},
			 {data: 'debit_in_usd_amount', name: 'debit_in_usd_amount'},
        ...
        "fnDrawCallback": function (oSettings) {
			...
			var sumIQDamount = data.reduce(function(sumAmount, item){
				// if(!isNaN(item.debit_in_iqd) && item.debit_in_iqd != null){
				// 	sumAmount   += parseFloat(item.debit_in_iqd); 
				// } 
				if(!isNaN(item.opening_balance) && item.opening_balance != null){
					sumAmount   += parseFloat(item.opening_balance); 
				} 
				return sumAmount;
			},0);
			
			$('#contact_table').find('span.totalAmountInIQD').text(sumIQDamount);

			var sumUSDamount = data.reduce(function(sumAmount, item){
				if(!isNaN(item.debit_in_usd) && item.debit_in_usd != null){
					sumAmount   += parseFloat(item.debit_in_usd);
				}
				return sumAmount;
			},0);
			$('#contact_table').find('span.totalAmountInUSD').text(sumUSDamount);
	...
	/* $(document).on('click', '.edit_contact_button', function(e){
		e.preventDefault();
    	$( "div.pay_contact_due_modal" ).load( $(this).attr('href'), function(){
    		$(this).modal('show');
    	});
    }); */
	...
	//Start: CRUD for contact  category
    //Category table
    var contact_category_table = $('#contact_category_table').DataTable({
					processing: true,
					serverSide: true,
					ajax: '/contact-categories',
					columnDefs: [ {
						"targets": 1,
						"orderable": false,
						"searchable": false
	...
	$(document).on('submit', 'form#contact_category_add_form', function(e){
		e.preventDefault();
		var data = $(this).serialize();
		...
		$.ajax({
			...
			success: function(result){
				if(result.success === true){
					$('div.category_modal').modal('hide');
					toastr.success(result.msg);
					contact_category_table.ajax.reload();
				} else {
					toastr.error(result.msg);
				}
			}
	...
	$(document).on('click', 'button.edit_category_button', function(){
    	...
    	$( "div.category_modal" ).load( $(this).data('href'), function(){
    		...
    		$('form#contact_category_edit_form').submit(function(e){
	    		e.preventDefault();
				var data = $(this).serialize();
