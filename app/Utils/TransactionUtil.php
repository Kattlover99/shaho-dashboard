...
//echo $il->logo;exit;
//echo Storage::url(config('constants.invoice_logo_path') . '/'.$il->logo);exit;
...
//            $output['total_due'] = ($due == 0) ? 0 : $this->num_f($due, $show_currency);
     ...
     * @return array
     */
    public function getSellTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $category_id = '')
    {
		...
		if (!empty($category_id)) {
			...
			->whereIn('contact_categories.id', $category_id);
		}
	...
	public function getProfitsSoldItemsForReport($business_id, $start_date = null, $end_date = null, $location_id = null, $category_id = '') {
		/*$query = TransactionSellLinesPurchaseLines::LeftJoin('purchase_lines as pl', 'pl.id', '=', 'transaction_sell_lines_purchase_lines.purchase_line_id')
			->LeftJoin('transaction_sell_lines as tsl', 'tsl.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')
			->LeftJoin('transactions as t', 'tsl.transaction_id', '=', 't.id')
			->where('t.business_id', '=', $business_id)
			->select(
				...
				'purchase_line_id',
				//DB::raw('((tsl.unit_price_inc_tax - pl.purchase_price_inc_tax) * transaction_sell_lines_purchase_lines.quantity) as profit')
				DB::raw('(tsl.unit_price - pl.purchase_price) as profit')
			);*/
		/*$query = TransactionSellLine::
					...
					->where('t.business_id', '=', $business_id)
					->select(
						DB::raw('(transaction_sell_lines.unit_price - purchase_lines.purchase_price) as profit')
					);
		...
		if (!empty($category_id)) {
			//dump($category_id);exit;
			$query->leftJoin('contacts', 't.contact_id', '=', 'contacts.id')
			->leftJoin('contact_categories', 'contacts.category', '=', 'contact_categories.id')
			->whereIn('contact_categories.id', $category_id);
		}
		...
		if (!empty($start_date) && !empty($end_date)) {
			//$query->whereBetween(DB::raw('date(transaction_date)'), [$start_date, $end_date]);
			$sells->whereDate('transaction_date', '>=', $start_date)->whereDate('transaction_date', '<=', $end_date);
		}
		
		if (!empty($category_id)) {
			//dump($category_id);exit;
			$sells->whereIn('contact_categories.id', $category_id);
		}
		
		$sells->groupBy('transactions.id');
		$sells->orderBy('transaction_date', 'desc');
		$sells->orderBy('invoice_no', 'desc');
		...
		foreach ($data as $row) {
			...
			//dump($row->final_total.'_'.$sell->sum('eachItemValue')).'<br/>';
			//$sum += $row->final_total-$sell->sum('eachItemValue');;
			$final_total += $row->final_total;;
			$eachItemValue += $sell->sum('eachItemValue');;
			
		}
		//echo $final_total.' - '.$eachItemValue;exit;
		return $final_total - $eachItemValue;
	}
...
//                        }
     ...
     * @return float
     */
    public function getTotalDiscounts($business_id, $transaction_type, $start_date, $end_date, $location_id = null, $category_id = '')
    {

        $query = Transaction::where('transactions.business_id', $business_id)
                    ->where('transactions.type', $transaction_type);
        ...
        $query->select(
            DB::raw("SUM(IF(transactions.discount_type = 'percentage', COALESCE(transactions.discount_amount, 0)*transactions.total_before_tax/100, COALESCE(transactions.discount_amount, 0))) as discount")
        );
    ...
    public function getTotalStockAdjustment($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
		...
		//$end_date = '2000-04-30';
		//$date = date("Y-m-d");
		$q = PurchaseLine::LeftJoin('transactions', 'purchase_lines.transaction_id', '=', 'transactions.id')
                ->select(
                    //'purchase_lines.purchase_price_inc_tax as dpp',
                    //'quantity', 'quantity_sold', 'quantity_adjusted',
                    DB::raw("(purchase_lines.quantity - purchase_lines.quantity_sold - purchase_lines.quantity_adjusted)*purchase_lines.purchase_price_inc_tax as total")
                    //DB::raw("purchase_lines.purchase_price_inc_tax as total")
                    //'exp_date'
                )
				
                //->where('purchase_lines.exp_date', '<', $date)
				->whereBetween(DB::raw('date(purchase_lines.exp_date)'), [$start_date, $end_date])
				//->whereNull('purchase_lines.exp_date')
                ->where('transactions.business_id', '=', $business_id)
	            ->where('transactions.status', '=', 'received')
				;
