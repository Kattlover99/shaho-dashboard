...
//            );
            ...
            //Get Total Inventory
	        $total_inventory = $this->transactionUtil->getTotalInventory(
	        	$business_id,
		        $start_date,
		        $end_date,
		        $location_id
	        ...
	        $profits_sold_items = $this->transactionUtil->getProfitsSoldItemsForReport($business_id, $start_date, $end_date, $location_id, $category_id);
	        //Discounts
	        $total_sell_discount = $this->transactionUtil->getTotalDiscounts($business_id, 'sell', $start_date, $end_date, $location_id, $category_id);
			
			$expireData = new ExpireData();
			$total_expire = $expireData->get_total_for_report($business_id, $start_date, $end_date);

            $data['total_inventory'] = !empty($total_inventory) ? $total_inventory : 0;
            ...
            $data['total_sell'] = (!empty($sell_details['total_sell_exc_tax']) ? $sell_details['total_sell_exc_tax'] : 0) - $data['total_sell_discount'];
	        ...
	        $data['total_expire'] = $total_expire;
	        $data['total_adjustment'] = !empty($total_stock_adjustment) ? $total_stock_adjustment : 0;
			//$data['total_profits_sold_items'] = (!empty($profits_sold_items) ? $profits_sold_items : 0) - $data['total_sell_discount'];
			$data['total_profits_sold_items'] = (!empty($profits_sold_items) ? $profits_sold_items : 0);

	        //$data['net_profit'] = $data['total_profits_sold_items'] - $data['total_expense'] - $data['total_adjustment'];
	        $data['net_profit'] = $data['total_profits_sold_items'] - $data['total_expense'] - $total_expire;

            return $data;
        }

        $business_locations = BusinessLocation::forDropdown($business_id, true);
        return view('report.profit_loss', compact('business_locations', 'contactCategory', 'business'));
    }
    ...
    public function getExpenseReport(Request $request)
    {
        ...
        $business_id = $request->session()->get('user.business_id');
        $filters = $request->only(['category', 'bank_id']);
        ...
        $business_locations = BusinessLocation::forDropdown($business_id, true);
        $banks = Bank::forDropdown($business_id, true);

        return view('report.expense_report')
                    ->with(compact('chart', 'categories', 'business_locations', 'banks'));
    }
    ...
    public function getStockExpiryReport(Request $request)
    {
        ...
        if ($request->ajax()) {
	        ...
	        if (!empty(request()->start_date) && !empty(request()->end_date)) {
		        $start = request()->start_date;
		        $end =  request()->end_date;
		        $query->where('exp_date', '<=', $end)
			        ->where('exp_date', '>=', $start);
            }
            ...
            if (!empty(request()->expired)) {
            	if (request()->expired == 'true') {
            		$query->where('exp_date', '<', date('Y-m-d'));
	            }
            }
            $report = $query->select(
                'p.name as product',
            ...
            return Datatables::of($report)
                ...
                ->addColumn('edit', function ($row) {
                    ...
                    if (!empty($row->exp_date)) {
                        $carbon_exp = \Carbon::createFromFormat('Y-m-d', $row->exp_date);
                        $carbon_now = \Carbon::now();
                        if ($carbon_now->diffInDays($carbon_exp, false) < 0 && $row->stock_left > 0.0) {
	                ...
	                if ($row->quantity_adjusted > 0.0) {
		                $html .=  ' <button type="button" class="btn btn-primary btn-xs recover_removed_items_btn" data-href="' . action('StockAdjustmentController@recoverExpiredStock', [$row->purchase_line_id]) . '"> <i class="fa fa-life-saver"></i> ' . __("lang_v1.recover_expired_items") .
			                '</button>';
	                }
                    return $html;
                })
            ...
            return Datatables::of($query)
                ->editColumn('product_name', function ($row) {
                 ...
                 ->editColumn('purchase_qty', function($row){
					 $purchase_qty = (float) $row->purchase_qty . ' ' . $row->unit;
                    return '<span class="purchase_qty_sum" data-orig-value="'.trim($purchase_qty).'">'.$purchase_qty.'</span>';
                 })
            ...
            return Datatables::of($query)
                ...
                ->editColumn('sell_qty', function($row){
                   // return (float) $row->sell_qty . ' ' .$row->unit;
					 $purchase_qty = (float) $row->sell_qty . ' ' .$row->unit;
                    return '<span class="purchase_qty_sum" data-orig-value="'.trim($purchase_qty).'">'.$purchase_qty.'</span>';
                 })
                 ->editColumn('subtotal', function($row){
                    return '<span class="display_currency" data-currency_symbol = true>' . $row->subtotal . '</span>';
